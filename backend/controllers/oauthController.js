...
const oauthConnect = async(req,res)=>{
    ...
    try {
        ...
        if(userCheck){
            ...
            if(state !== "none"){
                await subscription(userCheck, state, false, payed);
                subscribe = true;
            } 
           
            if(userCheck.phone){ 
                res.status(200).json({message : "login" ,subscribe, user : userCheck});
            }
            else{
                res.status(200).json({message : "editProfile" , user : userCheck, subscription : lastSubscription[0].id + 1});
            }
            ...
            bcrypt.hash(password, 10, async(err, passwordHash)=>{
                if(err){
                    ...
                    if(state !== "none"){
                        await subscription(user ,state, false,payed);
                    }
                    res.status(200).json({message : "editProfile", user : user , password : password , subscription : lastSubscription[0].id + 1})
                }
            });
        }
    } catch (error) {

...
const signUp = async(req,res) =>{
    const {username, password, fullname, email, phone, address, birthDate, gender, language, course} = req.body;
        ...
        bcrypt.hash(password, 10, async(err, passwordHash)=>{
            if(err){
                ...
                if(user){
                    ...
                    return res.status(201).json({message : "success", subscribe, user : user})
                }
                else{
                    return res.status(500).json({message : "Server Error"});
                }
